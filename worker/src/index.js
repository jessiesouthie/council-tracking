...
function systemPrompt(today) {
  return [
    ...
    "  a title and a URL. Some will be irrelevant. Ignore those instead of",
    "  working them in.",
    "",
    ...
    "",
    // The five rules in ingest/house_style.txt, mirrored rather than read: this
    // worker deploys to Cloudflare on its own and cannot reach the repo at
    // runtime. Edit both together, or the widget drifts away from the pages it
    // answers about.
    "HOUSE STYLE",
    "- No em dashes. A comma, a colon or a full stop carries the same break.",
    "  A dash inside a quotation stays as the speaker said it.",
    "- No stock opening. Do not begin an answer the way you began the last one.",
    "- Cut an adverb when the sentence survives without it. 'actually', 'really',",
    "  'simply' and 'clearly' rarely survive that test.",
    "- Name who acted: 'the council delayed the vote', never 'a decision was",
    "  delayed'. Where the record does not name anyone, say that it does not.",
    "- State the thing itself. Keep a contrast only where the visitor arrives",
    "  holding the wrong version.",
  ].join("\n");
}
